...
class TopicAligner:
    ...
    def load_merged_vectors(self, window):
        """加载合并后的主题向量"""
        vector_path = self.base_path / "output" / "lda" / "merged_topic_vectors" / f"{window}_merged_vectors.npy"
        return np.load(vector_path)

    def load_cluster_info(self, window):
        """加载聚类信息"""
        info_path = self.base_path / "output" / "lda" / "merged_topic_vectors" / f"{window}_cluster_info.pkl"
    ...
    def run(self):
        ...
        # 保存结果
        output_dir = self.base_path / "output" / "lda" / "alignment"
        output_dir.mkdir(parents=True, exist_ok=True)

        # 保存演化事件表
        evolution_df.to_csv(output_dir / 'topic_evolution_events.csv', index=False)

        # 保存对齐结果
        with open(output_dir / 'alignment_results.pkl', 'wb') as f:
            pickle.dump(alignment_results, f)

        # 保存演化分析
        with open(output_dir / 'evolution_analysis.pkl', 'wb') as f:
        ...
        logger.info("\\n✅ 主题对齐处理完成")
        logger.info("📁 输出文件:")
        logger.info(f"  - {output_dir / 'topic_evolution_events.csv'}")
        logger.info(f"  - {output_dir / 'alignment_results.pkl'}")
        logger.info(f"  - {output_dir / 'evolution_analysis.pkl'}")
